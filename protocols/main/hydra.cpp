...
pair<uint64_t, pair<uint64_t, uint64_t>> sumcheck(int lvl, vector<uint64_t> &z, uint64_t ri, pair<vector<uint64_t>, vector<uint64_t>> &challenges, int tmp_index){
  ...
  if(!HYDRA){
    ...
    uint64_t test_wire_add = mod(w1 + w2);
    uint64_t test_wire_mult = mod(modMult(w1, w2));
    uint64_t fz = 0;
    for(int k = 0; k < ni; ++k){
      if(c[lvl][k].type == 0){
        fz = mod(fz + modMult(beta, modMult(c[lvl][k].wire[tmp_index], test_wire_add)));
      }
      else if(c[lvl][k].type == 1){
        fz = mod(fz + modMult(beta, modMult(c[lvl][k].wire[tmp_index], test_wire_mult)));
      }
    }
...
void eval_circuit(){
  for(int i = 1; i < c.size(); ++i){
    ...
    for(int j = 0; j < c[i].size(); ++j){
      if(c[i][j].type == 0){
        c[i][j].val = mod(c[i-1][c[i][j].in1].val + c[i-1][c[i][j].in2].val);
      }
      else if(c[i][j].type == 1){
        c[i][j].val = modMult(c[i-1][c[i][j].in1].val, c[i-1][c[i][j].in2].val);
      }
    }
  }
}
...
int main(){
  cout << "INPUT: DEPTH WIDTH LATENCY(MS) HYDRA" << endl;
  cin >> DEPTH >> WIDTH >> LATENCY >> HYDRA;
