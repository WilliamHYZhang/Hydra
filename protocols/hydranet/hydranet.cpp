...
Implementation of the HydraNet protocol described in "Hydra: Pipelineable
...
Specialized neural network protocols based on Ghodsi's implementation described
in "SafetyNets: Verifiable Execuation of Deep Neural Network on an Untrusted
Cloud"
*/
...
{
  if (argc!=3)
    cout << "Enter the architecture file and subdepth as argument." << endl, exit(1);
  ifstream archfile(argv[1]);

  SUB_DEPTH = atoi(argv[2]);
  string line;
  ...
  for(int i = 0; i < L; i += SUB_DEPTH){
    vector<int*> sub(layers.begin()+(i==0 ? 0 : i-1), i+SUB_DEPTH >= layers.size() ? layers.end() : layers.begin()+i+SUB_DEPTH);
    pipeline.push_back(sub);
  }
  ...
  // layers increasing upwards, no effect on verification time
  ...
  for (int i = 0; i < pipeline.size(); ++i)
  {
    ...
    //commit w_in/w_out
    //random values for now, no effect on verification time
    string cmd = "cd ../../deps/poly-commit/target/release && ./hydra-cmd ";
    uint64 n = myPow(2,d);
    cmd += to_string(n) + " ";
    for(int i = 0; i < n; ++i){
      cmd += to_string(abs(rand())) + " ";
    }
    cmd += "1 ";
    cmd += to_string(d) + " ";
    for(int i = 0; i < d; ++i) {
